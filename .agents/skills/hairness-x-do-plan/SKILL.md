---
name: hairness-x-do-plan
description: Execute an accepted WorkPlan through bounded routes.
---

Invoke with `$hairness-x-do-plan`.
Surface: intent; chat-first.
Route: `hairness work do-plan`.

Fixed: `{"controls":{"persistence":"effect-checkpoint"}}`.

Defaults: `{"mode":"auto"}` unless overridden.
Build compact InvocationDraft. Set `draft.result`=`default`. Call `hairness invoke start --operation hairness/work:execute --draft-json - --json` before questions. `--auto` advances progress only. Ask returned gaps; else follow `preview.next`.
