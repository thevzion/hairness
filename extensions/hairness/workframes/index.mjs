...
const postures = new Set(['discuss', 'map', 'recap', 'propose', 'plan', 'act', 'execute'])
...
async function save(runtime, value, event) {
  ...
  await runtime.contracts.validate('WorkframeState', value)
...
async function segmentCommand(action, flags, runtime) {
  ...
  if (action === 'close') {
    ...
    if (artifact.owner !== 'hairness/workframes' || artifact.type !== 'segment-digest') throw new Error(`${flags.digest} is not a Workframes SegmentDigest.`)
...
async function artifactProducer(kind, runtime, value) {
  ...
  const route = { schemaVersion: 2, protocolVersion: '0.2', id: runId, kind: 'producer', requirement: 'required', resultSchema: 'ArtifactEnvelope', fanIn, workload: kind === 'recap' ? 'balanced' : 'deep' }
  ...
  const assignment = { schemaVersion: 2, protocolVersion: '0.2', id: `produce-${type}`, profile: 'producer', goal: kind === 'recap' ? 'Reduce the active segment into its minimum durable meaning.' : 'Turn accepted segment decisions into one executable bounded plan.', outcome: `Artifact work/${segment.id}-${kind} of type ${type}.`, workload: route.workload, budget: 1, inputs: [{ mission: value.mission }, { segment }, { frames: value.frames.filter((frame) => frame.segmentId === segment.id) }, { artifactContract: { id: `work/${segment.id}-${kind}`, type, owner: 'hairness/workframes', metadata: { labels: ['work'], signals: [kind], relations: [{ type: 'informs', target: { kind: 'segment', id: segment.id } }], freshness: { policy: 'manual' }, provenance: { kind: 'extension', id: 'hairness/workframes', version: '0.2.0-alpha.0' } }, requiredPayload: payload } }], targets: [], exclusions: ['target mutation', 'Git mutation', 'external source mutation', 'nested subagents', 'transcript storage'], allowedSources: ['artifact:read', 'workframes:read'], requestedEffects: [], result: { schema: 'ArtifactEnvelope', disposition: 'artifact', artifactOwner: 'hairness/workframes', artifactType: type } }
...
async function executePlan(action, flags, runtime, value) {
  ...
  if (artifact.owner !== 'hairness/workframes' || artifact.type !== 'work-plan') throw new Error(`${artifactId} is not a Workframes WorkPlan.`)
  ...
  return { summary: 'WorkPlan checkpoint is valid; resolve each step through its owning operation extension.', status: 'ready', plan: artifact, constraints, authority: [], limits: ['Workframes grants no effects itself.'], routes: artifact.payload.steps }
}
