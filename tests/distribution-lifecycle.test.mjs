...
test('team and forge payloads have distinct operational boundaries', async () => {
  ...
  for (const path of ['LICENSE', 'SPEC.md', 'SECURITY.md', 'ROADMAP.md', 'CHANGELOG.md', 'CONTRIBUTING.md', 'STATUS.md', 'docs', 'templates', 'providers', 'catalog', 'src/bootstrap', 'extensions/hairness/maintainer', 'extensions/hairness/initiative-controls', 'extensions/hairness/delivery-controls', 'scripts/check-commits.mjs', 'scripts/check-pack.mjs', 'scripts/impact-gate.mjs']) await assert.rejects(access(join(team.target, path)), path)
  ...
  await access(join(team.target, 'extensions/hairness/worktree-controls/extension.json'))
  ...
  const teamManifest = JSON.parse(await readFile(join(team.target, 'hairness.json'), 'utf8'))
  assert.equal(teamManifest.defaults.worktrees.directorySuffix, '-worktrees')
  ...
  await access(join(forge.target, 'extensions/hairness/initiative-controls/extension.json'))
  await access(join(forge.target, 'extensions/hairness/delivery-controls/extension.json'))
  await access(join(forge.target, 'extensions/hairness/worktree-controls/extension.json'))
  const forgeManifest = JSON.parse(await readFile(join(forge.target, 'hairness.json'), 'utf8'))
  assert.equal(forgeManifest.defaults.worktrees.cleanup, 'checkpoint')
  assert.ok(forgeManifest.extensions.some((extension) => extension.id === 'hairness/maintainer'))
  assert.ok(forgeManifest.extensions.some((extension) => extension.id === 'hairness/initiative-controls'))
  assert.ok(forgeManifest.extensions.some((extension) => extension.id === 'hairness/delivery-controls'))
  assert.ok(forgeManifest.extensions.some((extension) => extension.id === 'hairness/worktree-controls'))
