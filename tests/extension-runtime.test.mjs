...
async function writeExtension(root, base, id, options = {}) {
  ...
  await writeFile(join(path, 'extension.json'), JSON.stringify({ schemaVersion: 2, protocolVersion: '0.2', id, version: '0.2.0-alpha.0', summary: `Fixture extension for ${id}.`, category: 'ecosystem', tags: ['fixture'], maturity: 'experimental', readme: './README.md', module: './index.mjs', capabilities: ['./capabilities/fixture.json'], dependencies: options.dependencies ?? [], services: options.services ?? [], commands, providerCommands: commands.map((command) => ({ id: `${id.replace('/', '.')}.${command.namespace}`, name: command.namespace, summary: `Run ${command.namespace}.`, kind: 'capability', classification: 'specialized', surface: 'specialized', command: `hairness ${command.namespace}`, arguments: [], result: { schema: 'ContextPacket', disposition: 'response' }, resultId: 'default', operation: { capability: capabilityId, id: 'run' }, instructions: `./commands/${command.namespace}.md` })) }))
