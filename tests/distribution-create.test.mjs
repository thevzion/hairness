...
test('create collects one gap at a time and materializes standalone sources', async () => {
  ...
  assert.deepEqual(result.routes, [`cd ${target}`, 'npm exec -- hairness onboarding next'])
  ...
  assert.equal(manifest.extensions.length, 11)
  assert.ok(manifest.extensions.some((entry) => entry.id === 'hairness/worktree-controls'))
  assert.equal(manifest.defaults.worktrees.placement, 'anchor-sibling')
  assert.equal(manifest.defaults.worktrees.layout, '{repository}/{type}/{slug}')
  assert.equal(manifest.defaults.worktrees.enforcement, 'required')
  ...
  assert.match(readme, /npm exec -- hairness onboarding next/)
  assert.match(readme, /npm exec -- hairness build --check/)
  assert.doesNotMatch(readme, /^hairness\s/m)
...
test('minimal materializes only the core and cockpit without dormant catalogue behavior', async () => {
  ...
  assert.equal(manifest.defaults.worktrees, undefined)
