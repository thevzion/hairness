...
test('provider compiler emits the same active command set for Codex and Claude', async () => {
  ...
  assert.equal(manifest.commands.length, 35)
  assert.equal(new Set(manifest.commands.map((command) => command.name)).size, 35)
  ...
  assert.deepEqual(manifest.commands.filter((command) => command.surface === 'namespace').map((command) => command.name), ['hairness-help', 'hairness-work', 'hairness-codebase', 'hairness-source', 'hairness-worktree'])
