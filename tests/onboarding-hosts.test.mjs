...
test('onboarding reads declarative gaps without executing extension code before trust', async () => {
  ...
  await writeFile(join(extension, 'extension.json'), JSON.stringify({ schemaVersion: 2, protocolVersion: '0.2', id: 'fixture/pre-trust', version: '0.2.0-alpha.0', summary: 'Pre-trust fixture.', category: 'ecosystem', tags: ['fixture'], maturity: 'experimental', readme: './README.md', module: './index.mjs', capabilities: [], dependencies: [], commands: [], providerCommands: [], onboarding: [{ id: 'fixture.choice', phase: 'domain', question: 'Fixture choice?', options: [{ value: 'off', label: 'Off' }], preferenceKey: 'fixture.choice' }] }))
  await writeFile(join(extension, 'README.md'), '# fixture/pre-trust\n')
  const manifestPath = join(root, 'hairness.json')
  const manifest = JSON.parse(await readFile(manifestPath, 'utf8'))
