...
managed checkout and one writer lease for each plan from an anchor-owned pool:
`workspace/` for the distribution and `codebases/<id>/` for registered
repositories. The main session remains in the stable home and receives only
the logical repository reference, handle, proof and compact worker result.
