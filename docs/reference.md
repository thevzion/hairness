...
`create <home> [base-extension]` creates a Git repository, installs
`@hairness/onboarding`, optionally installs the base Extension, builds provider
projections, runs doctor and creates one initial commit. `init` writes a bare
Home without an Extension, build or commit.

## Extension

An installed Extension lives at
`extensions/<namespace>/<name>/hairness.json`. The `name` field must match the
...
`add` resolves all requested Extensions, validates every manifest and source
...
clean Extension in one transaction. A local modification blocks the transaction
...
`build` discovers `extensions/*/*/hairness.json`, composes instructions and
...
`doctor` validates runtime, Extensions, Targets, Integrations and generated
outputs. A customized Extension remains healthy after a matching build; missing
