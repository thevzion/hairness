...
async function inspected(root, id) {
  ...
  for (const definition of manifest.sources ?? []) for (const operation of definition.operations) assert.equal(typeof module.sourceOperations?.[operation.id], 'function', `${id} does not export source operation ${operation.id}`)
